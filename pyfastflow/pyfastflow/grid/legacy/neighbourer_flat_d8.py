"""
Vectorized 2D grid neighbouring operations for flow calculations with D8 connectivity.
...
@ti.func
def i_from_rc_d8(row: ti.i32, col: ti.i32):
    """
...
@ti.func
def which_edge_d8(i: ti.i32) -> ti.u8:
    """
...
@ti.kernel
def fill_edges_d8(edges: ti.template()):
    for i in edges:
        edges[i] = which_edge_d8(i)
...
@ti.func
def neighbour_n_d8(i: ti.i32, tdir: ti.template()):
    ...
    if tdir == 0:
        ...
        j = bottomright_n_d8(i)

    res = -1
    if validate_link_n_d8(i, tdir):
        res = j
    else:
        res = -1
    return res
...
@ti.func
def can_leave_domain_n_d8(i: ti.i32):
    """Check if flow can leave domain at this node."""
    return which_edge_d8(i) > 0
...
@ti.func
def topleft_pew_d8(i: ti.i32):
    """Get topleft neighbor with EW wrapping."""
    row, col = rc_from_i_d8(i)
    res = -1
    if col > 0:
        res = i - cte.NX - 1
    else:
...
@ti.func
def top_pew_d8(i: ti.i32):
    """Get top neighbor index."""
    return i - cte.NX


@ti.func
def topright_pew_d8(i: ti.i32):
...
@ti.func
def bottomleft_pew_d8(i: ti.i32):
...
@ti.func
def bottom_pew_d8(i: ti.i32):
    """Get bottom neighbor index."""
    return i + cte.NX


@ti.func
def bottomright_pew_d8(i: ti.i32):
...
@ti.func
def validate_link_pew_d8(i: ti.i32, tdir: ti.template()):
...
@ti.func
def neighbour_pew_d8(i: ti.i32, tdir: ti.template()):
    ...
    if tdir == 0:
        ...
        j = bottomleft_pew_d8(i)
    elif tdir == 6:
        j = bottom_pew_d8(i)
    else:
        j = bottomright_pew_d8(i)

    res = -1
    if validate_link_pew_d8(i, tdir):
        res = j
...
@ti.func
def can_leave_domain_pew_d8(i: ti.i32):
...
@ti.func
def left_pns_d8(i: ti.i32):
    """Get left neighbor index."""
    return i - 1


@ti.func
def right_pns_d8(i: ti.i32):
    """Get right neighbor index."""
    return i + 1


@ti.func
def bottomleft_pns_d8(i: ti.i32):
...
@ti.func
def bottomright_pns_d8(i: ti.i32):
...
@ti.func
def validate_link_pns_d8(i: ti.i32, tdir: ti.template()):
...
@ti.func
def neighbour_pns_d8(i: ti.i32, tdir: ti.template()):
    ...
    if tdir == 0:
        ...
        j = bottomleft_pns_d8(i)
    elif tdir == 6:
        j = bottom_pns_d8(i)
    else:
        j = bottomright_pns_d8(i)

    res = -1
    if validate_link_pns_d8(i, tdir):
        res = j
    else:
        res = -1
    return res
...
@ti.func
def can_leave_domain_pns_d8(i: ti.i32):
...
@ti.func
def topleft_custom_d8(i: ti.i32):
    """Get topleft neighbor with custom wrapping."""
    node = -1
    tb = cte.boundaries[i]
    if tb == 9:
        node = topleft_pns_d8(i) if which_edge_d8(i) <= 3 or which_edge_d8(i) >= 6 else topleft_pew_d8(i)
    elif tb > 0:
        node = topleft_n_d8(i)
...
@ti.func
def topright_custom_d8(i: ti.i32):
    """Get topright neighbor with custom wrapping."""
    node = -1
    tb = cte.boundaries[i]
    if tb == 9:
        node = topright_pns_d8(i) if which_edge_d8(i) <= 3 or which_edge_d8(i) >= 6 else topright_pew_d8(i)
    elif tb > 0:
        node = topright_n_d8(i)
...
@ti.func
def bottomleft_custom_d8(i: ti.i32):
    """Get bottomleft neighbor with custom wrapping."""
    node = -1
    tb = cte.boundaries[i]
    if tb == 9:
        node = bottomleft_pns_d8(i) if which_edge_d8(i) <= 3 or which_edge_d8(i) >= 6 else bottomleft_pew_d8(i)
    elif tb > 0:
        node = bottomleft_n_d8(i)
...
@ti.func
def bottom_custom_d8(i: ti.i32):
    """Get bottom neighbor with custom wrapping."""
    node = -1
    tb = cte.boundaries[i]
    if tb == 9:
        node = bottom_pns_d8(i)
    elif tb > 0:
        node = bottom_n_d8(i)
...
@ti.func
def bottomright_custom_d8(i: ti.i32):
    """Get bottomright neighbor with custom wrapping."""
    node = -1
    tb = cte.boundaries[i]
    if tb == 9:
        node = bottomright_pns_d8(i) if which_edge_d8(i) <= 3 or which_edge_d8(i) >= 6 else bottomright_pew_d8(i)
    elif tb > 0:
        node = bottomright_n_d8(i)
...
@ti.func
def validate_link_custom_d8(i: ti.i32, tdir: ti.template()):
    ...
    if tb == 0:
        ...
        res = validate_link_n_d8(i, tdir)
    else:
        edge = which_edge_d8(i)
        if edge == 0:
            res = validate_link_n_d8(i, tdir)
        elif edge <= 3 or edge >= 6:
            res = validate_link_pns_d8(i, tdir)
        else:
            res = validate_link_pew_d8(i, tdir)
    return res


@ti.func
def neighbour_custom_d8(i: ti.i32, tdir: ti.template()):
    """Get neighbor with custom boundary validation."""
    j: ti.i32 = -1
    if tdir == 0:
        j = topleft_custom_d8(i)
    elif tdir == 1:
        j = top_custom_d8(i)
    elif tdir == 2:
        j = topright_custom_d8(i)
    elif tdir == 3:
        j = left_custom_d8(i)
    elif tdir == 4:
        j = right_custom_d8(i)
    elif tdir == 5:
        j = bottomleft_custom_d8(i)
    elif tdir == 6:
        j = bottom_custom_d8(i)
    else:
        j = bottomright_custom_d8(i)

    res = -1
    if validate_link_custom_d8(i, tdir) and j > -1:
        res = j
...
@ti.func
def can_leave_domain_custom_d8(i: ti.i32):
...
def topright_d8(i: ti.i32):
    """Get topright neighbor - switches between boundary modes."""
    res = -1
    if ti.static(cte.BOUND_MODE == 0):
        res = topright_n_d8(i)
    elif ti.static(cte.BOUND_MODE == 1):
        res = topright_pew_d8(i)
    elif ti.static(cte.BOUND_MODE == 2):
        res = topright_pns_d8(i)
    elif ti.static(cte.BOUND_MODE == 3):
        res = topright_custom_d8(i)
    return res


def left_d8(i: ti.i32):
    ...
    if ti.static(cte.BOUND_MODE == 0):
        ...
        res = left_custom_d8(i)
    return res


def right_d8(i: ti.i32):
    ...
    if ti.static(cte.BOUND_MODE == 0):
        ...
        res = right_custom_d8(i)
    return res


def bottomleft_d8(i: ti.i32):
    """Get bottomleft neighbor - switches between boundary modes."""
    res = -1
    if ti.static(cte.BOUND_MODE == 0):
        res = bottomleft_n_d8(i)
    elif ti.static(cte.BOUND_MODE == 1):
        res = bottomleft_pew_d8(i)
    elif ti.static(cte.BOUND_MODE == 2):
        res = bottomleft_pns_d8(i)
    elif ti.static(cte.BOUND_MODE == 3):
        res = bottomleft_custom_d8(i)
    return res


def bottom_d8(i: ti.i32):
    ...
    if ti.static(cte.BOUND_MODE == 0):
        ...
        res = bottom_custom_d8(i)
    return res


def bottomright_d8(i: ti.i32):
    """Get bottomright neighbor - switches between boundary modes."""
    res = -1
    if ti.static(cte.BOUND_MODE == 0):
        res = bottomright_n_d8(i)
    elif ti.static(cte.BOUND_MODE == 1):
        res = bottomright_pew_d8(i)
    elif ti.static(cte.BOUND_MODE == 2):
        res = bottomright_pns_d8(i)
    elif ti.static(cte.BOUND_MODE == 3):
        res = bottomright_custom_d8(i)
    return res


def validate_link_d8(i: ti.i32, tdir: ti.template()):
    ...
    if ti.static(cte.BOUND_MODE == 0):
        ...
        res = validate_link_custom_d8(i, tdir)
    return res


def neighbour_d8(i: ti.i32, tdir: ti.template()):
    """
    ...
    res = -1
    if ti.static(cte.BOUND_MODE == 0):
        res = neighbour_n_d8(i, tdir)
    elif ti.static(cte.BOUND_MODE == 1):
        res = neighbour_pew_d8(i, tdir)
    elif ti.static(cte.BOUND_MODE == 2):
        res = neighbour_pns_d8(i, tdir)
    elif ti.static(cte.BOUND_MODE == 3):
        res = neighbour_custom_d8(i, tdir)

    return res


def can_leave_domain_d8(i: ti.i32):
    """
    ...
    res = -1
    if ti.static(cte.BOUND_MODE == 0):
        res = can_leave_domain_n_d8(i)
    elif ti.static(cte.BOUND_MODE == 1):
        res = can_leave_domain_pew_d8(i)
    elif ti.static(cte.BOUND_MODE == 2):
        res = can_leave_domain_pns_d8(i)
    elif ti.static(cte.BOUND_MODE == 3):
        res = can_leave_domain_custom_d8(i)
    return res


# Main API functions - automatically switch based on cte.BOUND_MODE
def compile_neighbourer_d8():
    global topleft_d8, top_d8, topright_d8, left_d8, right_d8, bottomleft_d8, bottom_d8, bottomright_d8, validate_link_d8, neighbour_d8, can_leave_domain_d8

    topleft_d8 = ti.func(topleft_d8)
    top_d8 = ti.func(top_d8)
    topright_d8 = ti.func(topright_d8)
    left_d8 = ti.func(left_d8)
    right_d8 = ti.func(right_d8)
    bottomleft_d8 = ti.func(bottomleft_d8)
    bottom_d8 = ti.func(bottom_d8)
    bottomright_d8 = ti.func(bottomright_d8)
    validate_link_d8 = ti.func(validate_link_d8)
    neighbour_d8 = ti.func(neighbour_d8)
    can_leave_domain_d8 = ti.func(can_leave_domain_d8)


@ti.kernel
def flow_out_nodes_d8(outnodes: ti.template()):
    for i in outnodes:
        outnodes[i] = can_leave_domain_d8(i)