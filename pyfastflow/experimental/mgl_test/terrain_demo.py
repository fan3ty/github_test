...
def _init_taichi_prefer_discrete():
    """Initialize Taichi preferring discrete GPU backends.

    Order of preference:
    - CUDA (typically NVIDIA dGPU)
    - Vulkan (often dGPU-capable on AMD/NVIDIA)
    - Metal (macOS discrete when applicable)
    - OpenGL (may bind to iGPU depending on system)
...
# Initialize Taichi with a preference for discrete GPUs
_init_taichi_prefer_discrete()

# -------- Prototype configuration (tweak here) --------
# Maximum mesh resolution along the longest side (vertex grid)
MESH_MAX_DIM = 2048
# Displacement (heightmap) resolution; can differ from mesh resolution
DISP_NX = 4096
DISP_NY = 4096
# ------------------------------------------------------
...
class TerrainRenderer:
    ...
    def __init__(self, width: int, height: int, ctx: Optional[moderngl.Context] = None,
                 mesh_max_dim: int = MESH_MAX_DIM, disp_nx: int = DISP_NX, disp_ny: int = DISP_NY):
        self.width = width
        self.height = height
        self.mesh_size = 4.0  # Physical size of the mesh in world units
        # Externalized resolutions
        self.mesh_max_dim = int(mesh_max_dim)
        self.hm_nx = int(disp_nx)
        self.hm_ny = int(disp_ny)
        ...
        self.sphere_mode: bool = False
        self.sphere_radius: float = 1.5
    ...
    def _create_shaders(self):
        ...
        void main() {
            ...
            // Ensure upward/outward orientation: use dV x dU
    ...
    def _generate_mesh(self):
        ...
        # Only texcoords are required by the shader in sphere/flat modes
    ...
    def _update_heightmap(self):
        """Generate new heightmap using PyFastFlow Perlin noise (kept as float32)."""
        heightmap = pff.noise.perlin_noise(
            self.hm_nx, self.hm_ny,
        ...
        # Keep CPU copy as float32 for accurate sampling
        self.heightmap_cpu = heightmap_normalized.astype(np.float32, copy=False)
        # Upload as GL_R32F
        if hasattr(self, 'heightmap_texture'):
            self.heightmap_texture.release()
        self.heightmap_texture = self.ctx.texture(
            (self.hm_nx, self.hm_ny), 1, self.heightmap_cpu.tobytes(), dtype='f4'
        )
    ...
    def _world_to_grid(self, x: float, z: float) -> Optional[Tuple[float, float]]:
        nx, ny = self.hm_nx, self.hm_ny
        half_x = float(getattr(self, 'mesh_size_x', self.mesh_size)) * 0.5
        half_z = float(getattr(self, 'mesh_size_z', self.mesh_size)) * 0.5
        if x < -half_x or x > half_x or z < -half_z or z > half_z:
            return None
        u = (x / (2.0 * half_x) + 0.5) * (nx - 1)
        v = (z / (2.0 * half_z) + 0.5) * (ny - 1)
        return u, v
    ...
    def intersect_ray(self, ray_origin: np.ndarray, ray_dir: np.ndarray) -> Optional[np.ndarray]:
        ...
        if self.sphere_mode:
            # Approximate by intersecting base sphere, then offset by displacement
            # Solve |ro + t*rd| = R
            ...
            t_hit = None
            for t in (t0, t1):
                if t > 0.0 and (t_hit is None or t < t_hit):
                    t_hit = t
            if t_hit is None:
                return None
            p = ro + rd * t_hit
            # Refine to displaced radius using heightmap sample along direction
            dirv = p / max(1e-6, np.linalg.norm(p))
            uv = self._dir_to_uv(dirv)
            h = self._sample_height_uv(uv[0], uv[1]) if uv is not None else 0.0
            R2 = R + float(h)
            # Recompute intersection for displaced radius
            c2 = np.dot(ro, ro) - R2*R2
            disc2 = b*b - c2
            if disc2 >= 0.0:
                ...
                t_hit2 = None
                for t in (t0, t1):
                    if t > 0.0 and (t_hit2 is None or t < t_hit2):
                        t_hit2 = t
                if t_hit2 is not None:
                    return ro + rd * t_hit2
            return p
        else:
            # Planar displaced heightfield ray-march
            t = 0.0
    ...
    def _dir_to_uv(self, dirv: np.ndarray) -> Optional[Tuple[float, float]]:
        d = dirv.astype(np.float32)
        n = np.linalg.norm(d)
        if n == 0:
            return None
        d /= n
        lon = math.atan2(float(d[2]), float(d[0]))  # [-pi, pi]
        lat = math.asin(float(d[1]))                # [-pi/2, pi/2]
    ...
    def _sample_height_uv(self, u: float, v: float) -> float:
        if self.heightmap_cpu is None:
            return 0.0
        nx, ny = self.hm_nx, self.hm_ny
        grid = self.heightmap_cpu
        u_grid = u * (nx - 1)
        v_grid = v * (ny - 1)
        i0 = int(math.floor(u_grid))
        j0 = int(math.floor(v_grid))
        i1 = min(i0 + 1, nx - 1)
        j1 = min(j0 + 1, ny - 1)
        du = u_grid - i0
        dv = v_grid - j0
    ...
    def render(self, view_matrix: np.ndarray, projection_matrix: np.ndarray, eye: np.ndarray, hide_underside: bool = True):
        ...
        # Provide mesh extents for flat mode and radius/switch for sphere mode
        mesh_size_x = float(getattr(self, 'mesh_size_x', self.mesh_size))
        mesh_size_z = float(getattr(self, 'mesh_size_z', self.mesh_size))
        try:
            self.program['u_mesh_size_x'] = mesh_size_x
            self.program['u_mesh_size_z'] = mesh_size_z
            self.program['u_sphere_mode'] = 1.0 if self.sphere_mode else 0.0
            self.program['u_sphere_radius'] = float(self.sphere_radius)
        except Exception:
            pass
    ...
    def _generate_mesh(self):
        """Generate a 2D mesh (vectorized) using heightmap aspect ratio.

        The longest mesh side uses `self.mesh_max_dim`, and the other side
        is derived from `hm_aspect = self.hm_nx / self.hm_ny`.
        """
        ...
        # Vertex positions (x, z) in [-mesh_size/2, mesh_size/2]
        u = np.linspace(0.0, 1.0, nx, dtype=np.float32)
        v = np.linspace(0.0, 1.0, ny, dtype=np.float32)
        U, V = np.meshgrid(u, v, indexing='xy')  # shapes (ny, nx)
        # World-space extents should match heightmap aspect ratio
        # Longest side uses self.mesh_size; the other scales by hm_aspect
        if hm_aspect >= 1.0:
        ...
        X = (U - 0.5) * self.mesh_size_x
        Z = (V - 0.5) * self.mesh_size_z
        Y = np.zeros_like(X, dtype=np.float32)

        vertices = np.stack([X, Y, Z], axis=-1).reshape(-1, 3).astype(np.float32, copy=False)
        texcoords = np.stack([U, V], axis=-1).reshape(-1, 2).astype(np.float32, copy=False)

        # Triangle indices (two per quad), vectorized
        ii = np.arange(nx - 1, dtype=np.uint32)
        jj = np.arange(ny - 1, dtype=np.uint32)
        II, JJ = np.meshgrid(ii, jj, indexing='xy')  # (ny-1, nx-1)
        tl = JJ * nx + II
        tr = tl + 1
        bl = tl + nx
        br = bl + 1
        ...
        # Upload buffers (use contiguous arrays)
        # Only texcoords are required by the shader
        self.vbo_texcoords = self.ctx.buffer(np.ascontiguousarray(texcoords))
        self.ibo = self.ctx.buffer(np.ascontiguousarray(indices))
    ...
    def _update_heightmap(self):
        ...
        # Keep CPU copy for ray intersection, as float32
        self.heightmap_cpu = heightmap_normalized.astype(np.float32, copy=False)

        # Create/update heightmap texture as 32-bit float (GL_R32F)
        if hasattr(self, 'heightmap_texture'):
            self.heightmap_texture.release()
...
class TerrainApp(mglw.WindowConfig):
    ...
    def __init__(self, **kwargs):
        ...
        self.renderer = TerrainRenderer(w, h, ctx=self.ctx,
                                        mesh_max_dim=MESH_MAX_DIM,
                                        disp_nx=DISP_NX, disp_ny=DISP_NY)
        self.live_noise_update = False
        self.hide_underside = False
        # UI state for noise parameters (decoupled from renderer for non-live edits)
        self.ui_noise = {
            'frequency': float(self.renderer.noise_params['frequency']),
            'octaves': int(self.renderer.noise_params['octaves']),
            'persistence': float(self.renderer.noise_params['persistence']),
        }
    ...
    def _evt_mouse_scroll(self, x_offset: float, y_offset: float):
        # Forward to imgui and unify behavior with main scroll handler
        try:
            self.imgui.mouse_scroll_event(x_offset, y_offset)
        except Exception:
            pass
        self._evt_counts['scroll'] += 1
        # Apply the same zoom-to-mouse behavior used in mouse_scroll_event
    ...
    def mouse_scroll_event(self, x_offset: float, y_offset: float):
        # Forward to imgui and apply unified zoom behavior
        try:
            self.imgui.mouse_scroll_event(x_offset, y_offset)
        except Exception:
            pass
    ...
    def _apply_zoom_to_mouse(self, y_offset: float):
        """Zoom towards/away from the point under the mouse cursor.

        - Cast a ray through the mouse position.
        - Intersect against terrain; fall back to plane y=0 if needed.
        - Move the orbit target towards the hit point proportionally to zoom.
        - Scale camera distance exponentially for smooth dolly.
        """
    ...
    def on_render(self, time: float, frame_time: float):
        ...
        # Work on local UI state so values persist without applying
        changed = False
        f = float(self.ui_noise['frequency'])
        logf = math.log10(max(1e-8, f))
        ch_logf, logf = imgui.slider_float("frequency (log10)", logf, -2.0, 2.0, '%.2f')
        f = float(10.0 ** logf)
        imgui.text(f"frequency = {f:.3f}")
        o = int(self.ui_noise['octaves'])
        ch_o, o = imgui.slider_int("octaves", o, 1, 10)
        p = float(self.ui_noise['persistence'])
        ...
        if ch_logf:
            self.ui_noise['frequency'] = f
            changed = True
        if ch_o:
            self.ui_noise['octaves'] = int(o)
            changed = True
        if ch_p:
            self.ui_noise['persistence'] = float(p)
            changed = True
        # Reflect pending changes even if slider not moving
        pending = (
            abs(self.ui_noise['frequency'] - float(self.renderer.noise_params['frequency'])) > 1e-8 or
            int(self.ui_noise['octaves']) != int(self.renderer.noise_params['octaves']) or
            abs(self.ui_noise['persistence'] - float(self.renderer.noise_params['persistence'])) > 1e-8
        )
        changed = changed or pending
        if ch_hs:
        ...
        if imgui.button("Apply noise changes") and changed:
            self.renderer.update_noise_params(
                frequency=float(self.ui_noise['frequency']),
                octaves=int(self.ui_noise['octaves']),
                persistence=float(self.ui_noise['persistence']),
            )
            changed = False
        if self.live_noise_update and (ch_logf or ch_o or ch_p):
            self.renderer.update_noise_params(
                frequency=float(self.ui_noise['frequency']),
                octaves=int(self.ui_noise['octaves']),
                persistence=float(self.ui_noise['persistence']),
        ...
        ch_sp, sp = imgui.checkbox("Sphere mode (planet)", bool(self.renderer.sphere_mode))
        if ch_sp:
            self.renderer.sphere_mode = bool(sp)
        sr = float(self.renderer.sphere_radius)
        ch_sr, sr = imgui.slider_float("Sphere radius", sr, 0.1, 10.0, '%.2f')
        if ch_sr:
            self.renderer.sphere_radius = float(sr)
        imgui.end()
